...
**注意:** INBOX の廃止は**出荷済み機能の削除**で、リポジトリの持ち主が単独の質問に対して「Crew が INBOX を兼ねる」を選んだことに基づく（spec の「合意の経緯」）。ここを勝手に広げないこと。
...
- 既定 SVG のクオリティ向上（髪型 1 種類のみ）。持ち主が「この絵で実装に進む」と判断済みで、絵は差し替え可能なので後戻りしない
- v1.6.1 としてのリリース判断
...
Task 1〜13 は全ステップ実行し、それぞれコミットがある（`0f53b48..3333704`、および表の #6 `0397621`）。
Task 14 は**部分的にのみ実施**。実施済みと未実施の内訳は Task 14 の各項目に記載した。

### 見つかった欠陥（**6 件**。テストは全部緑のまま通り抜けたもの）
...
| 6 | 表示名とルーティングキーが同じフィールドを共有していて、キャラの名前欄に `worker:main` のような不透明なキーが出る | **機能追加の依頼を受けて `launch.ts` を読んだとき**（`leaf(…, p.label)`）。画面で観測したわけではなく、コードから予測した | `0397621` |
...
上の表の「どう見つけたか」列を数えるとこうなる。

- **6 件すべて、差分やコードを読むことで見つかった**
  （実装者の自己申告をコードで裏取り 2 / レビューエージェント 1 / 最終レビュー 2 / 機能追加時のコード読み 1）
- **Task 14（実機で見て直す）が拾った欠陥は 0 件**
- そして Task 14 Step 2 は **5 項目中 4 項目が未実施のまま出荷**された

つまり「テストが届かない層」を Architecture で先に名指ししたことは正しかったが、
**その層を実際に塞いだのは目視タスクではなく、1 タスクごとの敵対的な差分レビュー**だった。
目視タスクは**書いただけでは 1 件も獲らない**。plan に的を書いて安心してはいけない。

（当初この節には「Task 14 が Critical を含む欠陥を拾った」と書いていた。
plan が予告した的と実際に出た欠陥が一致していたので、そう推測した。**符合であって因果ではなかった。**
「どう見つけたか」列を作ったことで露見した）
...
このリポジトリは public。docs から持ち主の個人名を汎用表現へ置換したが、
**消えたのは HEAD だけで、公開履歴には全部残っている**（`4ec5ea8` などを `git show` すれば読める）。
履歴の書き換えは v1.7.0 タグと fork/clone を壊すため、代償が釣り合わないと判断して行っていない。
**「消した」ではなく「以後の版には載せない」が実際の状態。**
